...
class Department:
    ...
    def __str__(self):
        return f"No products in the department {self.name}"
...
class Store:
    ...
    # add a __str__ method so that we can observe our store instances
    def __str__ (self):
        return f"Store {self.name}, {self.location}, {self.departments} "


store = Store("LambdaStore", 44.13444, -122.3342, ["Clothing", "Cookware", "Books", "Sporting Goods"])

print(store)

# we want to add departments
# let's take input from the user and have them specify departments
# by the department index in the list
...
while True:
    ...
    try:
        ...
        #user didn't give us a value that could be cast to a number
